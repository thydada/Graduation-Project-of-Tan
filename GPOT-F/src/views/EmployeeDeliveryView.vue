      ...
      <table class="package-table">
        ...
        <tbody>
          <tr v-for="pkg in packages" :key="pkg.id">
            ...
            <td>{{ pkg.weight }}</td>
            <td>{{ formatDate(pkg.updateTime) }}</td>
            <td>
              <span class="status-badge status-transporting">{{ pkg.status }}</span>
            </td>
            <td class="actions">
              <button
                class="btn btn-success"
                @click="handleDeliver(pkg.id)"
                :disabled="processingId === pkg.id"
              >
                {{ processingId === pkg.id ? '处理中...' : '送达' }}
              </button>
            </td>
          </tr>
  ...
  setup() {
    ...
    const fetchPackages = async () => {
      ...
      try {
        ...
        const response = await api.getTransportingPackages(employeeId)
    ...
    const handleDeliver = async (packageId) => {
      ...
      try {
        const employeeId = getCurrentEmployeeId()
        const response = await api.deliverPackage(packageId, employeeId)

        if (response.data.success) {
          successMessage.value = '送达成功！包裹状态已更新为待取件'
          // 移除已送达的包裹
          ...
          errorMessage.value = response.data.message || '送达失败'
        }
      } catch (error) {
        console.error('送达失败:', error)
        errorMessage.value = error.response?.data?.message || '送达失败，请稍后重试'
      } finally {
